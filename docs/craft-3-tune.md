...
You probably think it's a good idea to disable headers that expose which the PHP version and CMS you use. And we think so too!
...
In the `config/general.php` you can disable the header with `'sendPoweredByHeader' => false,` (default: true). This is not required, since we strip all X-Powered-By headers eventually.
